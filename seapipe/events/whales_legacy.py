...
from numpy import (asarray, array2string, concatenate, char, empty, nan, ndarray, sum, zeros)
    ...
    def whale_it(self, method, cat, cycle_idx=None, adap_bands=False, 
        ...
        INPUTS:
            ...
            duration ->  Tuple of 2 digits, for the minimum and maximum duration of any 
                         detected events.
            subs ->      List of participants to perform detection on e.g. ['sub-SP001']
                         Can also be set to 'all' (string) at which it will run on all
                         the participants it can find in rec_dir
            sessions ->  List of sessions to perform detection on e.g. ['V1']
                         Can also be set to 'all' (string) at which it will run on all
                         the sessions it can find in rec_dir/<sub>/
        ...
        for i, sub in enumerate(subs):
            ...
            if sessions == 'all':
                sessions = listdir(rec_dir + '/' + sub)
                sessions = [x for x in sessions if not '.' in x]   
            
            for v, ses in enumerate(sessions):
                logger.debug(f'Commencing participant {sub}, session {ses}')
                tracking[f'{sub}'][f'{ses}'] = {'spindles':{}} 
    
                ## c. Load recording
                rdir = rec_dir + sub + '/' + ses + '/eeg/'
                try:
                ...
                if type(chan) == DataFrame:
                    ...
                    if chans.size == 0:
                        logger.warning(f"Session not found in column 'sub' in tracking.tsv' for {sub}, {ses}.")
                        flag+=1
                        break
                    chans = chans.filter(regex='chanset')
                    chans = chans.dropna(axis=1)
                    if chans.size == 0:
                        logger.warning(f"No reference channel set found in tracking.tsv' for {sub}, {ses}, skipping...")
                        flag+=1
                        break
                else:
                    chans = chan
                   
                if type(ref_chan) == DataFrame:
                    ...
                    ref_chans = ref_chans.dropna(axis=1)
                    if ref_chans.size == 0:
                        logger.warning(f"No reference channel set found in tracking.tsv' for {sub}, {ses}, skipping...")
                        flag+=1
                        break
                ...
                if type(chans) == list:
                    if type(ref_chans) == DataFrame and ref_chans.size >1:
                        logger.error("SETUP ERROR: channels are hardcoded, but there were more than 2 reference channel sets found in 'tracking.tsv'. For channel setup options, refer to documentation:")
                        logger.info('https://seapipe.readthedocs.io/en/latest/index.html')
                        flag+=1
                        break
                    elif type(ref_chans) == DataFrame:
                        ref_chans=ref_chans.to_numpy()
                        ref_chans = ref_chans.astype(str)
                        ref_chans = char.split(ref_chans[0], sep=', ')
                        ref_chans = ref_chans[0]
                    chanset = {key: ref_chans for key in chans}
                
                elif type(chans) == DataFrame:
                    if type(ref_chans) == DataFrame and ref_chans.size != chans.size:
                        logger.error(f"SETUP ERROR: there must be the same number of channel sets and reference channel sets in 'tracking.tsv', but for {sub}, {ses}, there were {chan.size} channel sets and {ref_chan.size} reference channel sets. For channel setup options, refer to documentation:")
                        logger.info('https://seapipe.readthedocs.io/en/latest/index.html')
                        flag+=1
                        break
                    elif type(ref_chans) == DataFrame:
                    ...
                    chans=chans.to_numpy() 
                    chans = chans.astype(str)
                    chans = char.split(chans[0], sep=', ')
                    chans = chans[0]  
                    chanset = {key: ref_chans for key in chans}
                else:
                    logger.error("The variable 'chan' should be a [list] or definied in the 'refset' column of 'tracking.csv' - NOT a string.")
                    flag+=1
                    break
                ...
                for c, ch in enumerate(chanset):
                    logger.debug(f'Reading data for {sub}, {ses}, ' + str(ch))
                    try:
                        segments = fetch(dset, annot, cat=cat, stage=stage, cycle=cycle, reject_epoch=True, 
                                      reject_artf=['Artefact', 'Arou', 'Arousal'])
                        segments.read_data([ch], ref_chan=chanset[ch], grp_name=grp_name)
                    ...
                    for m, meth in enumerate(method):
                        ...
                        if cat[0] == 1 and cat[1] == 0:
                            for s, seg in enumerate(segments):
                                logger.debug(f'Analysing stage {stage[s]}')
                                spindles = detection(seg['data']) # detect spindles
                                annot.remove_event(name=meth, time=None, chan=ch) # remove any previously detected spindles on channel first
                            ...
                            for s, seg in enumerate(segments):
                                logger.debug('Analysing cycle {} of {}, stages: {}'.format(s + 1, 
                                      len(segments),stage))
                                spindles = detection(seg['data'])
                                annot.remove_event(name=meth, time=None, chan=ch)
            ...
            for i, p in enumerate(part):
                ...
                for v, vis in enumerate(visiti):  
                    ...
                    if keyword is not None:
                        ...
                        xml_file_path = xml_dir + '/' + p + '/' + vis + r'/' + xml_file[0]
