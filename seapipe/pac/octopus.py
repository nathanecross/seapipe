...
class octopus:
    ...
    def pac_it(self, cycle_idx, cat, nbins, filter_opts, 
                 ...
                 peaks = None, adap_bw = 4, invert = False, 
            ...
            for v, ses in enumerate(sessions):
                ...
                for c, ch in enumerate(chanset):
                    ...
                    if not chanset[ch]:
                        ...
                        logchan = chanset[ch]
                    logger.debug(f"Using phase frequency band: {round(f_amp[0],2)}-{round(f_amp[1],2)} Hz for {sub}, {ses}, {str(ch)}:{'-'.join(logchan)}")    
                    ...
                    # 6.a. Divide segments based on concatenation
                    nsegs=[]
                    ...
                    for sg in range(len(nsegs)):
                        ...
                        # g. put data in blocks (for surrogate testing)
                        longpha[sg//50, sg%50] = pha
                        longamp[sg//50, sg%50] = amp
                        
                        # h. put data in long format (for preferred phase)
                        ampbin[sg, :] = mean_amp(pha, amp, nbins=nbins)
    
                        # i. if number of events not divisible by block length,
                        ...
                        # l. Create bins for preferred phase
                        vecbin = zeros(nbins)
                        width = 2 * pi / nbins
                        for n in range(nbins):
                            vecbin[n] = n * width + width / 2  
                        
                        # m. Calculate mean direction (theta) & mean vector length (rad)
                        ...
                        outputfile = outputfile.split('_pac_parameters.csv')[0] + '_mean_amps.p'
