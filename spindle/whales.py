    ...
    for i, p in enumerate(part):
        ...
        if i<1:
            ...
            if cat[0] == 1 and cat[1] == 0:
                detable=zeros((len(part),len(visit)*len(chan)*len(method)*len(stage)))
            else:
                ...
                for m, meth in enumerate(method):
                    ...
                    if cat[0] == 1 and cat[1] == 0:
                        for s, seg in enumerate(segments):
                            ...
                            detable[i,v*(len(chan)+len(method)+len(stage))+c+m+s-1] = len(spindles.events)
                            if i<1:
